"""Training pipeline for XLM-RoBERTa-large with GlobalPointer."""
...
def resolve_device(requested: str) -> torch.device:
    """Resolve `auto` and reject unavailable explicit CUDA requests."""
...
def train_epoch(
    ...
    *,
    ...
    """Train one epoch and return the mean batch loss."""
    ...
    for batch_index, batch in enumerate(progress, start=1):
        ...
        with _autocast(device, amp_enabled):
            loss = global_pointer_loss(batch["span_labels"], _model_logits(model, batch))
...
def evaluate_loss(
    ...
    *,
    ...
    """Calculate mean GlobalPointer loss on a tokenized split."""
...
def run(config: dict[str, Any], context: ExperimentContext) -> Path:
    ...
    _set_seed(seed, seed_cuda=device.type == "cuda")

    raw_train = read_jsonl(context.path(config, "train"), require_entities=True)
    raw_dev = read_jsonl(context.path(config, "dev"), require_entities=True)
    train_records, train_preprocessing = preprocess_records(raw_train)
    dev_records, dev_preprocessing = preprocess_records(raw_dev)
    ...
    train_records, augmentation_stats = apply_cluster_augmentation(
        ...
        num_clusters=int(augmentation.get("num_clusters", 16)),
        max_candidates=int(augmentation.get("max_candidates", 3)),
        seed=seed,
    )
    ...
    total_updates = math.ceil(len(train_loader) / accumulation_steps) * epochs
    ...
    model_metadata: JsonObject = {
        "schema_version": 1,
        "architecture": "XLMRobertaGlobalPointer",
        ...
        "max_length": max_length,
        "stride": stride,
        "preprocessing": train_preprocessing["method"],
    }
    ...
    print(f"Preprocessing train/dev: {train_preprocessing} / {dev_preprocessing}")
    print(f"Cluster augmentation: {augmentation_stats}")
    ...
    best_model, best_tokenizer, saved = load_model_bundle(model_dir, device)
    final_predictions = predict_records(
        best_model,
        best_tokenizer,
        dev_records,
        max_length=int(saved["max_length"]),
        stride=int(saved["stride"]),
        batch_size=inference_batch_size,
        threshold=float(saved["span_threshold"]),
        device=device,
    )
    ...
    summary: JsonObject = {
        **model_metadata,
        ...
        "best_epoch": best_epoch,
        "best_dev_micro_f1": best_dev_f1,
        "preprocessing": {"train": train_preprocessing, "dev": dev_preprocessing},
